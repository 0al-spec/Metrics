...
It is an observed quantity under the active pricing surface, not a structural
prediction about future maintenance or change cost.
...
Unlike $C_{\mathrm{inf}}$, $C_{\mathrm{verify}}$, and $C_{\mathrm{build}}$,
this quantity is not observed spend. It is a structural proxy derived from
graph topology, verifiability, and stability signals.
...
to the future change-cost proxy). $C_{\mathrm{own}}$ is not an estimate guarantee
and should not be interpreted as direct monetary spend. It is an economic lens
for comparing structural alternatives and observing drift under AI-mediated
construction.
...
Every reported economic artifact should preserve its provenance: pricing surface
identifier, model and tool versions, time window, currency or unit convention,
and whether each displayed quantity is observed spend or a derived proxy.
...
\item \textbf{Scope economics (non-normative totals):}
  the current selection $S$ reports aggregated economics
  \begin{equation}
  \mathrm{ScopeTotals}(S) \Leftarrow
